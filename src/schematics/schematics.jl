...
"""
    add_node!(g::SchematicGraph, comp::AbstractComponent; base_id=name(comp), id=nothing, kwargs...)
...
If `id::Symbol` is provided, it is used as the node's id exactly, and an error is thrown
if it is already in use (or would clash with the unique-naming scheme, e.g. `:foo_0` when
`uniquename` would normalize it). Explicit ids are registered with the graph's name
counter, so later auto-generated ids will not collide with them.
...
function add_node!(
    ...
    node_id = if isnothing(id)
        uniquename(base_id, '_', counter=g.namecounter)
    else
        # Register the explicit id with the same counter used for auto-generated ids.
        # If registration would alter it, the id is either already in use or would
        # collide with a future auto-generated name — reject rather than silently
        # renaming a node the caller expects to address by id.
        registered = uniquename(String(id), '_', counter=g.namecounter)
        registered == String(id) || error(
            "Node id :$id is not available in SchematicGraph \"$(g.name)\" " *
            "(uniquename would rename it to \"$registered\"). Explicit node ids " *
            "must be unused and stable under unique-naming."
        )
        String(id)
    end
    node = ComponentNode(node_id, comp)
    g.node_dict[Symbol(node_id)] = node
    push!(g.nodes, node)
    add_vertex!(g, props)
    return node
end
