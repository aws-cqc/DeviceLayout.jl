...
| `Heal` | `Heal(:clean, :metal); Heal(:clean, :metal)` fails on the second call because the renamed internal PG already exists in `:clean`. | `Heal(:metal); Heal(:metal)` executes twice using the same internal PG name. The second heals the already-healed result and should be geometrically idempotent. | `Heal(:clean, :metal); Heal(:cleaner, :clean)` preserves the internal name suffix while changing the layer prefix twice. |
