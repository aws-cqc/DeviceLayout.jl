...
metadata into layout files. This is implemented in the `Texts` module. These are saved to GDS as TEXT records, which may be displayed differently by different GDS viewers. Graphical export of text elements may also be system-dependent, so if you want a fixed geometry, you should use `PolyText`.

See [API Reference: Texts](@ref api-texts).

## PolyText

DeviceLayout.jl provides a few styles for rendering text as polygons. [`polytext!`](@ref) will render a string
to a cell with options that depend on the style. The [`DotMatrix`](@ref) style provides more options than
