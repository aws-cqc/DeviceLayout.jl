...
@testitem "Mesh sizing for imported models" setup = [CommonTestSetup] begin
    ...
    # The shape a JSON parser returns for an externally written control-point file.
    doc = Dict(
        "points" => [
            Dict(
                "h_um" => 1.0,
...
@testitem "SolidModelComponent fusion modes and group_map" setup = [CommonTestSetup] begin
    using .SchematicDrivenLayout
    import DeviceLayout.SchematicDrivenLayout: SolidModelComponent
    import DeviceLayout.SolidModels: gmsh, dimtags, hasgroup, dimgroupdict
    using DeviceLayout: SemanticMeta, GDSMeta, Rectangle, centered

    # External CAD as .xao so it carries a face group: a 40 μm cube resting on z = 0, with its
    # top face in a group named "lid".
    _tmp = SolidModel("smcm_author"; overwrite=true)
    gmsh.model.set_current("smcm_author")
    b = gmsh.model.occ.addBox(-20, -20, 0, 40, 40, 40)
    gmsh.model.occ.synchronize()
    _tmp["cube"] = [(3, b)]
    _tmp["lid"] = [
        (2, t) for
        (_, t) in gmsh.model.getEntitiesInBoundingBox(-21, -21, 39, 21, 21, 41, 2)
    ]
    xao = joinpath(tdir, "smcm_cube.xao")
    DeviceLayout.save(xao, _tmp)
    gmsh.model.remove()

    tech = ProcessTechnology(
        (; chip_outline=GDSMeta()),
        (; thickness=(; chip_outline=[50μm]), chip_thicknesses=[50μm])
    )
    target = SolidModelTarget(
        tech;
        levelwise_layers=[:chip_outline],
    ...
    # Substrate extruded downward from a chip-outline rectangle, cube resting on its top plane.
    function build(fusion)
        reset_uniquename!()
        g = SchematicGraph("smcm_$fusion")
        add_node!(
            g,
            SolidModelComponent(
                xao,
        ...
        fp = plan(g; log_dir=nothing)
        render!(
            fp.coordinate_system,
            centered(Rectangle(200μm, 200μm)),
            SemanticMeta(:chip_outline)
        )
        check!(fp)
        sm = SolidModel("smcm_$fusion"; overwrite=true)
        render!(sm, fp, target)
        return sm
    end
    faceset(sm, grp) = Set(
        abs(t) for
        (_, t) in gmsh.model.getBoundary(dimtags(sm[grp, 3]), false, false, false)
    )
    shared(sm) = intersect(faceset(sm, "cad_1"), faceset(sm, "chip_outline_L1_extrusion"))

    @testset ":targeted fuses locally and keeps mapped file groups" begin
        sm = build(:targeted)
        @test hasgroup(sm, "cad_1", 3)
        @test hasgroup(sm, "cad_cube", 3)        # file group, renamed by group_map
        @test hasgroup(sm, "cad_lid", 2)
        @test length(shared(sm)) == 1            # one shared interface face
        @test length(dimtags(sm["cad_lid", 2])) == 1
    end

    @testset ":none imports after the pass and leaves the part unfused" begin
        sm = build(:none)
        @test hasgroup(sm, "cad_cube", 3)
        @test isempty(shared(sm))                # touching, but no shared entity
    end

    @testset ":global still fuses through the global pass" begin
        sm = build(:global)
        @test length(shared(sm)) == 1
    end

    @test_throws ArgumentError SolidModelComponent(xao, SemanticMeta(:x); fusion=:sideways)

    @testset "an invalid mode set through the keyword constructor is caught at render" begin
        # The @compdef keyword constructor does not validate `fusion`, so the check has to
        # happen where the mode is consumed.
        bad = SolidModelComponent{typeof(1.0μm)}(;
            filename=xao,
            meta=SemanticMeta(:chip_outline),
            fusion=:sideways
        )
        g = SchematicGraph("smcm_bad")
        add_node!(g, bad)
        fp = plan(g; log_dir=nothing)
        @test_throws ArgumentError SchematicDrivenLayout.component_solidmodel_ops(
            fp,
            target
        )
    end
